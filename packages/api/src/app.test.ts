...
describe("kite-research-agent API", () => {
  it("returns health", async () => {
    const response = await app.request("/health");
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ ok: true, service: "kite-research-agent" });
  });

  it("lists primary briefs", async () => {
  ...
  it("exposes product metadata on a single /meta route", async () => {
    ...
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body.service).toBe("kite-research-agent");

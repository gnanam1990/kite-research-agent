...
# Stage 00 — Context and Repo Setup for KiteResearch Agent
...
## Goal
Create the initial monorepo for **KiteResearch Agent**: Autonomous research analyst for Kite mainnet, wallets, contracts, products, and ecosystem activity.

This stage should establish a clean foundation only. Do not build all product modules yet.

## Product constraints
- Product name: `KiteResearch Agent`
- Repo name: `kite-research-agent`
- Header subtitle: `RESEARCH`
- Positioning: Continuously monitor Kite on-chain activity and ecosystem signals, summarize changes, produce daily briefs, and trigger alerts.
...
- Health route: `GET /health` returns `{ ok: true, service: "kite-research-agent" }`.
...
# Stage 01 — Database and Domain Types for KiteResearch Agent
...
## Goal
Design and implement the domain model, database schema, migrations, seed data, and TypeScript types for **KiteResearch Agent**.
...
- `ResearchEvent{id, source, event_type, entity_address, tx_hash, payload_json, importance_score, created_at}`
- `EntityIntelligence{id, entity_type, address, summary_markdown, facts_json, updated_at}`
...
- `ResearchBrief{id, period_start, period_end, title, content_markdown, sources_json, created_at}`
- `ResearchAlert{id, owner_address, name, rule_json, channel_json, active}`
- `ResearchDelivery{id, alert_id, event_id, status, attempts, error, delivered_at}`
...
# Stage 02 — Backend API for KiteResearch Agent
...
## Goal
Build the Hono API surface for **KiteResearch Agent** using the schema from Stage 01.
...
# Stage 03 — Worker and Agent Runtime for KiteResearch Agent
...
## Goal
Build the background worker/runtime layer for **KiteResearch Agent**. This product should feel productive and agentic, not just CRUD screens.
...
- Wallet/Contract Intelligence
- Ecosystem Product Tracker
...
# Stage 04 — Frontend Shell and Kite Theme for KiteResearch Agent
...
## Goal
Build the frontend shell for **KiteResearch Agent** with the attached Kite design system. Do not improvise the visual style.
...
## Required layout
- Sticky `SiteHeader` with Kite logo and `RESEARCH` subtitle.
...
# STAGE_05_MODULE_01_ON_CHAIN_EVENT_WATCHER.md

# Stage 05 — Module 1: On-chain Event Watcher for KiteResearch Agent
...
## Goal
Implement the **On-chain Event Watcher** module for **KiteResearch Agent**.

## Module purpose
Monitor blocks, txs, wallets, contracts, and token transfers for notable activity.
...
- Importance score is ESTIMATED
- Watchlist filters reduce noise
...
# STAGE_06_MODULE_02_WALLET_CONTRACT_INTELLIGENCE.md

# Stage 06 — Module 2: Wallet/Contract Intelligence for KiteResearch Agent
...
## Goal
Implement the **Wallet/Contract Intelligence** module for **KiteResearch Agent**.

## Module purpose
Generate readable profiles and changes for wallets/contracts.
...
## Acceptance criteria
- Summaries cite underlying tx hashes/records
- No unsupported claims
- Risk notes are cautious and evidence-based
...
# STAGE_07_MODULE_03_ECOSYSTEM_PRODUCT_TRACKER.md
...
# STAGE_08_MODULE_04_DAILY_RESEARCH_BRIEF_GENERATOR.md

# Stage 08 — Module 4: Daily Research Brief Generator for KiteResearch Agent
...
## Goal
Implement the **Daily Research Brief Generator** module for **KiteResearch Agent**.

## Module purpose
Create daily/weekly Kite research reports from events and product changes.
...
## Acceptance criteria
- Brief includes source event links
- AI-generated interpretations are labeled
...
# STAGE_09_MODULE_05_ALERTS_WEBHOOK_DELIVERY.md

# Stage 09 — Module 5: Alerts + Webhook Delivery for KiteResearch Agent
...
## Goal
Implement the **Alerts + Webhook Delivery** module for **KiteResearch Agent**.

## Module purpose
Notify users when research conditions match.
...
## Acceptance criteria
- Webhook payload includes event and summary
- Failed deliveries retry
- HMAC signing supported
...
# Stage 10 — Kite Wallet and Payment Integration for KiteResearch Agent
...
## Goal
Add safe Kite chain integration to **KiteResearch Agent**. This stage should wire KiteScan, RPC, wallet connection, address views, tx verification, and payment-safety patterns where relevant.
...
# Stage 11 — Testing, Security, and Deployment for KiteResearch Agent
...
## Goal
Harden **KiteResearch Agent** so it is not just a demo. Add tests, security checks, deployment docs, environment examples, and operational safety.
...
# Stage 12 — Final Polish, README, and Demo Script for KiteResearch Agent
...
## Goal
Prepare **KiteResearch Agent** for a public-quality GitHub repo and product demo.
